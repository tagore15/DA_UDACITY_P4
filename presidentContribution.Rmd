...
ggplot(data=subset(df_clean, contbr_city %in% ct & cand_nm %in% nm), aes(x=cand_nm, fill = contbr_city)) + geom_bar() + scale_y_log10() + theme(axis.text.x=element_text(angle=90))
```
...
ggplot(data=subset(df_clean, contbr_occupation %in% occu), aes(contbr_occupation, contb_receipt_amt)) + geom_boxplot() + theme(axis.text.x=element_text(angle=90))
```
...
ggplot(data=subset(df_clean, contbr_occupation %in% occu & cand_nm %in% nm), aes(cand_nm, fill=contbr_occupation)) + geom_bar() + theme(axis.text.x=element_text(angle=90))
```
...
ggplot(data=subset(df_clean, contbr_occupation %in% occu & contbr_city %in% ct), aes(contbr_city, fill=contbr_occupation)) + geom_bar() + theme(axis.text.x=element_text(angle=90))
```
...
ggplot(data=subset(df_clean, contbr_city %in% ct & cand_nm %in% nm & contb_receipt_amt > 0), aes(x=cand_nm, y=contb_receipt_amt/1e+06, fill=contbr_city)) + geom_bar(stat='identity') + theme(axis.text.x=element_text(angle=90))

df_clean <- dplyr::arrange(df_clean, contbr_occupation, cand_nm)
ggplot(data=subset(df_clean, contbr_city %in% ct & cand_nm %in% nm & contbr_occupation %in% occu & contb_receipt_amt > 0), aes(x=cand_nm, y=contb_receipt_amt, fill=contbr_occupation)) + geom_bar(stat='identity') + theme(axis.text.x=element_text(angle=90))
```
...
ggplot(data=subset(df_clean, contbr_city %in% ct & cand_nm %in% nm & contb_receipt_amt > 0), aes(x=datef, y=contb_receipt_amt)) + geom_point(aes(color=cand_nm))
```
...
tmp <- subset(df_clean, cand_nm %in% nm) %>% group_by(cand_nm, contbr_city) %>% summarise(n=sum(contb_receipt_amt)) %>% group_by(cand_nm) %>% arrange(desc(n)) %>% slice(1:6)
ggplot(data=tmp, aes(x=contbr_city,  y=n)) + geom_bar(stat='identity') + facet_wrap(~cand_nm, scales="free") + theme(axis.text.x = element_text(angle = 90))
```
...
ggplot(data=subset(df_clean, contbr_occupation %in% occu & cand_nm %in% nm), aes(cand_nm, fill=contbr_occupation)) + geom_bar() + theme(axis.text.x=element_text(angle=90)) + xlab("candidate name") + ylab("number of contributions") + ggtitle("Number of contributions versus candidate name/occupation")
```
...
ggplot(data=subset(df_clean, contbr_city %in% ct & cand_nm %in% nm), aes(x=cand_nm, fill = contbr_city)) + geom_bar() + scale_y_log10() + theme(axis.text.x=element_text(angle=90)) + xlab("candidadate name") + ylab("log of number of contributions") + ggtitle("log10(number of contributions) versus candidate/city")
```
...
ggplot(data=subset(df_clean, contbr_city %in% ct & cand_nm %in% nm & contb_receipt_amt > 0), aes(x=datef, y=contb_receipt_amt)) + geom_point(aes(color=cand_nm), alpha = 0.2) + xlab("date") + ylab("contribution amount in dollars") + ggtitle("contribution amount versus date")
```
...
Most of contribution were concentrated on few candidates, cities and occupations. To make data easier to explore and tractable, I found top candidate, cities and occupation and used them in analysis.
